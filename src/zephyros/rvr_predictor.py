...
def learn_and_predict(learn_data: pd.DataFrame, predict_data: pd.DataFrame,
                      ...
                      rvm_options: dict|None=None) -> tuple:
    """
...
def learn(x_in: np.ndarray, y_in: np.ndarray,
          rvm_options: dict|None=None):
    """
...
def predict(model: EMRVR, scaler: tuple, x: np.ndarray) -> tuple:
    """
